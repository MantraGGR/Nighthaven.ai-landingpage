...
export const Hero = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/send-demo-email", {
        method: "POST",
        headers: {
      ...
      setStatus("Something went wrong. If you want to book an appointment, or call, or demo,please email mantra.nighthaven@gmail.com");
    }
  };
    ...
    <section className="relative pt-32 lg:pt-36">
      <Container className="flex flex-col lg:flex-row gap-10 lg:gap-12">
        ...
        <div className="relative flex flex-col items-center text-center lg:text-left lg:py-8 lg:items-start lg:max-w-none max-w-3xl mx-auto lg:mx-0 lg:flex-1 lg:w-1/2">
          ...
          {status && (
            <p className="text-sm mt-3 text-gray-500 lg:text-left text-center">{status}</p>
          )}
        </div>
